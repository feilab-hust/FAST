...
for i = 1:iter
            ...
            for view = 1:view_num
                ...
                for uu=1:Nb
                        for vv=1:Nb
                            ...
                            corr_map=gather(normxcorr2(sub_fpj,sub_img));  
    ...
    for view = 1:view_num        
            if DAO>0 && i>1
                ...
                [coordinate1,coordinate2]=meshgrid(1:size(img,1),1:size(img,2));         
                    img_temp=interp2(coordinate1,coordinate2,img(:,:,view),coordinate1+map_wavshape_yy,coordinate2+map_wavshape_xx,'cubic',0); 
            else
                img_temp=img(:,:,view);
            end
            H_temp = H(:,:,view,:);
    ...
    if mod(i,10)==0 && i~=0
        recon = gather(recon);
        str_save=fullfile(save_Path,sprintf('deconv_iter%d_%s',i,img_name));
        recon = recon .^ 0.85;
        write3d(recon,str_save,16)
    end
end