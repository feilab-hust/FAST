...
def no_background_patches(threshold=0.4, percentile=99.9):
    ...
    def _filter(datas, patch_size, dtype=np.float32):
        ...
        return filtered > threshold * np.percentile(image,percentile)  # returns a binary mask of the same size
    return _filter
...
def create_patches(
    ...
    if normalization is None:
        ...
        def _min_max(x, eps=1e-7):

            max_ = np.max(x)
            min_ = np.min(x)
            return (x - min_) / (max_ - min_ + eps)
        normalization = lambda patches_x, patches_y, x, y, mask, channel: (_min_max(patches_x), _min_max(patches_y))
